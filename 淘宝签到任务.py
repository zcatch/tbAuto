...
def check_in_task():
    ...
    if package == "com.taobao.taobao" and (d(className="android.widget.TextView", text="做任务赚元宝").exists or d(className="android.widget.TextView", text="红包签到")):
        return True
    return False
...
def find_coin_btn():
    sign_btn = d(className="android.view.View", description="红包签到")
    if sign_btn.exists:
        print("点击红包签到按钮")
        d.double_click(sign_btn[0].center()[0], sign_btn[0].center()[1])
        time.sleep(10)
    else:
        raise Exception("没有找到红包签到按钮")


def find_earn_btn(stop=True):
    earn_btn = d(className="android.widget.Button", textContains="赚元宝")
    if earn_btn.exists:
        earn_btn.click()
        time.sleep(5)
    else:
        continue_btn = d(className="android.widget.Button", text="继续领钱")
        if continue_btn.exists:
            d.click(continue_btn[0].bounds()[0] - 50, continue_btn[0].center()[1])
            time.sleep(5)
        else:
            if stop:
...
if close_btn and close_btn.exists:
    ...
    time.sleep(3)
find_coin_btn()
earn_btn = d(className="android.widget.Button", textMatches="立即签到")
if earn_btn.exists:
    earn_btn.click()
    time.sleep(10)
    to_btn = d(className="android.widget.TextView", textMatches="立即领.*元宝")
    if to_btn.exists:
        to_btn.click()
        start_time = time.time()
        while True:
            ...
            time.sleep(3)
        d.press("back")
get_btn = d(className="android.widget.Button", textMatches="点击领取")
if get_btn.exists:
    get_btn.click()
    time.sleep(5)
find_earn_btn()
